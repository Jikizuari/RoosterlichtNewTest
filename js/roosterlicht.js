...
var roosterlicht = roosterlicht || {};
// Android succesfully registered
roosterlicht.successHandler = function( result ) {
  alert( 'result: ' + result );
};

// Android + iOS, error while registering
roosterlicht.errorHandler = function( error ) {
  alert( 'error: ' + error );
};
...
roosterlicht.onNotificationAPN = function( event ) {
  ...
  if ( event.badge ) {
    pushNotification.setApplicationIconBadgeNumber( roosterlicht.successHandler, roosterlicht.errorHandler, event.badge );
  }
};
...
// iOS token handler
roosterlicht.tokenHandler = function( token ) {
